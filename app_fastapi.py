...
from api import meta_async as meta_api
...
# FIX: Access the .aio property for async support
...
# Create a Jinja2 Environment for rendering templates from strings
# You can configure autoescape and other options as needed
...
async def get_gemini_response_with_context(
    user_message: str,
    context: str,
    sender_id: str,
    ...
    return await get_gemini_response(message, sender_id, history, config)
...
async def get_gemini_response(
    ...
    try:
        chat_session = await chat_sessions.async_get_session(sender_id, history)
        ...
        response = await chat.send_message(user_message, config=config)
...
async def get_gemini_response_with_context_json(
    user_message: str,
    context: str,
    sender_id: str,
    history: List[genai_types.Content] = None,
    ...
    """
    message = f'Context: """{context}"""\n\n{user_message}'
    return await get_gemini_response_json(message, sender_id, history, config)

async def get_gemini_response_with_context_json_rag(
    user_message: str,
    context: str,
    sender_id: str,
    history: List[genai_types.Content] = None,
    ...
    results = await context_controller.async_query_similarity(user_message)
    # 3. Prepare the context for the LLM
    context = "\n---\n".join(results) if results and results.get('documents') else "No relevant context found."
...
async def get_gemini_response_json(
    user_message: str,
    sender_id: str,
    history: List[genai_types.Content] = None,
    config: Dict = None,
    ...
    # actually generate response:
    try:
        chat_session = await chat_sessions.async_get_session(sender_id, history, tools=tools)
        if chat_session == None:
            return None
        ...
        chat: Chat = chat_session["chat"]  # type: ignore
        _response = await chat.send_message(user_message, config=config)
        ...
        try:
            ...
            if function_call.name == "retrieve_testas_information":
                ...
                _response.text = await get_gemini_response_with_context_json_rag(query=query_arg)
...
async def get_new_conversation_context(sender_id, object_type):
    """
    Get the context of a new conversation with a user.
    """
    # Get all messages between the page and the user
    conversation = await meta_api.get_conversation_messages_by_user_id(sender_id)
    if not conversation:
        return ""
    ...
    batch_messages = await meta_api.batch_get_messages_by_ids_v2(message_ids, object_type)
    
    return batch_messages

async def get_conversation_label(sender_id, object_type):
    conversation = await meta_api.get_conversation_messages_by_user_id(sender_id)
    if not conversation:
        return ""
    
    # Get the labels of the conversation
    labels = await meta_api.get_labels_of_conversation(sender_id, object_type)
...
# ===== === === === === === === CORE LOGICS
async def get_and_send_message(sender_id, messages : Message, object_type):
    print("[Webhook]: Get and send message", sender_id, messages)
    
    # send typing indicator, we don't need to await it because it's not critical
    await meta_api.send_typing_indicator(sender_id)
    ...
    if not chat_sessions.is_session_exist(sender_id):
        batch_messages = await get_new_conversation_context(sender_id, object_type)
        if batch_messages:
    ...
    if reply_context:
        ...
        bot_response = await get_gemini_response_with_context_json(
            user_message,
            reply_context,
            sender_id,
        ...
        bot_response = await get_gemini_response_json(
            user_message,
            sender_id,
            history=chat_history,
            config=g_gemini_config,
    ...
    # FIX 2: Replace thread_utils.delayed_call with asyncio.sleep
    # This keeps the logic non-blocking for the server but delays the send for the user
    if typing_time > 0:
        await asyncio.sleep(typing_time)

    if bot_reply:
        await meta_api.send_meta_message(sender_id, bot_reply, object_type)
    ...
    if image_urls and image_send_threshold > 0.5: 
        ...
        await meta_api.send_meta_image(sender_id, image_url, object_type=object_type)

# === === === === === === === ROUTING FUNCTION
async def handle_user_feedback(sender_id, user_message, object_type):
    feedback_text = user_message[len("/feedback"):].strip()
    await feedback_controller.async_log_feedback_text(object_type, sender_id, feedback_text)
    await meta_api.send_meta_message(sender_id, "Cảm ơn bạn đã góp ý! 💬", object_type) # ✅ 

async def handle_user_message(message_event, object_type):
    # get time
    ...
    if user_message.lower().startswith("/feedback"):
        ...
        await handle_user_feedback(sender_id, user_message, object_type)
        return
    ...
    # get_labels_of_conversation = await get_conversation_label(sender_id, object_type)
    ...
    if reply != None:
        ...
        reply_message_text = await meta_api.get_message_by_id(message_id, object_type)
    ...
    def debounce_callback(uid, msgs):
        ...
        if (msgs):
            # Use run_coroutine_threadsafe to call async function from sync callback
...
async def handle_reaction_event(event, object_type):
    ...
    message = await meta_api.get_message_by_id(message_id, object_type)

    if action == "react":
        reaction_type = event["reaction"]["reaction"]
        emoji = event["reaction"]["emoji"]
        # Reaction added
        await feedback_controller.async_log_feedback(object_type, sender_id, message_id, message, reaction_type, emoji)
    elif action == "unreact":
        # Reaction removed
        await feedback_controller.async_remove_feedback(message_id)
...
async def read_config(request: Request):
    context = {**g_gemini_config, **g_app_config, "request": request}
    return templates.TemplateResponse(
        request=request, name="config.html", context=context
    )
...
async def update_config(request: Request):
    ...
    for key in form:
        if key not in CONFIG_FIELD_TYPE_MAP:
            continue

        field_value = form.get(key, "")
        field_type, field_default = CONFIG_FIELD_TYPE_MAP[key]
        field_value = common.safe_cast(field_value, field_type, field_default)
        
        if key.startswith("gemini_"):
            clean_key = key.removeprefix("gemini_")
            g_gemini_config[clean_key] = field_value
        elif key.startswith("app_"):
            clean_key = key.removeprefix("app_")
            g_app_config[clean_key] = field_value

    success = _apply_app_config()
    if success:
        print(f"[Config] Successfully change config.")

    context = {"request": request, "name": "FastAPI User", **g_gemini_config, **g_app_config}
    return templates.TemplateResponse(
        request=request, name="config.html", context=context
    )

@app.get("/htop/{interval}")
async def htop(interval:int):
...
async def update_context(file: UploadFile = File(...)):
    """
    ...
    try:
        ...
        if not isinstance(data, list):
            raise HTTPException(status_code=400, detail="JSON file must contain a list of documents.")

        # 4. Add documents to the repository
        all_chunks = text_chunking(data)
        
        # Note: Ensure context_controller.add_documents is compatible with async or run it in a threadpool if it's blocking
        await context_controller.async_add_documents(
            documents=[chunk['content'] for chunk in all_chunks],
            metadatas=[{'source_url': chunk['source_url'], 'title': chunk['title'], 'chunk_id': chunk['chunk_id']} for chunk in all_chunks]
        )

        return {"message": f"Successfully added {len(data)} documents to the repository."}

    except json.JSONDecodeError:
        raise HTTPException(status_code=400, detail="Invalid JSON format.")
    except Exception as e:
        print(f"Error updating context: {e}")
        raise HTTPException(status_code=500, detail="An error occurred while updating the context.")
...
async def verify_webhook(request: Request):
    ...
    # Return error with specific status code
        ...
        for message_event in entry.get("messaging", []):
            if "message" in message_event:
                ...
                if (is_bot_message(app_id, sender_id, object_type) and is_echo == True):
                    ...
                    await handle_user_message(message_event, object_type)
            elif "reaction" in message_event:
                    await handle_reaction_event(message_event, object_type)
    
    print("[Webhook]: Finished processing")
    return PlainTextResponse(content="ok") # Default is 200
